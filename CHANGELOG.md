...
- latent-source: read-only E01/EWF support through a libewf FFI binding, opening single and multi-segment images as one logical source and surfacing the acquisition hash; libewf is an optional dependency, and the crate builds against a stub that returns a clear error when it is absent
